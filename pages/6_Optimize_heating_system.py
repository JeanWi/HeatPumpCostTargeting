...
with col1:
    ...
    st.markdown("**Electricity profile specifications**")
...
with col2:
    ...
    st.markdown("**Heat pump capacity factor**")
    cf = st.number_input("Average capacity factor of heat pump", value=0.9, min_value=0.0, max_value=1.0,)
...
p_gen["heat_demand"] = cf
allowable_cost_noflex = calculate_allowable_investment_cost(
    p_gen["heat_demand"],
    p_gen["p"] / 1000,
    p_th / 1000,
    T_l,
...
st.write(f"Allowable investment costs: {round(allowable_cost_noflex, 2)} EUR/kW")
...
if flexibility_model == "Demand side response":
    l = st.number_input("Max. shift duration (hours)", value=5, min_value=1, max_value=72, step=1)
elif flexibility_model == "Heat storage":
    storage_cap = st.number_input("Storage capacity (hours of average demand)", value=10, min_value=0, max_value=168, step=1)
...
if st.button("Solve"):
    ...
    if flexibility_model == "Demand side response":
        ...
        # storage variables
        m.var_charge_balance = pyo.Var(m.set_t, within=pyo.Reals)
        m.var_soc = pyo.Var(m.set_t, within=pyo.NonNegativeReals, bounds=[0, storage_cap * p_gen["heat_demand"].mean()])

        # storage constraints
        def init_soc(const, t):
            if t >=1:
                return m.var_soc[t] == m.var_soc[t-1] + m.var_charge_balance[t]
            else:
                return m.var_soc[t] == m.var_soc[max(m.set_t)] + m.var_charge_balance[t]
        m.con_soc = pyo.Constraint(m.set_t, rule=init_soc)

        def init_flex_energybalance(const, t):
            return m.var_demand_flex[t] == m.par_demand[t] + m.var_charge_balance[t]
        m.con_flex_energybalance = pyo.Constraint(m.set_t, rule=init_flex_energybalance)
    ...
    st.write(f"Allowable investment costs (no flexibility): {round(allowable_cost_noflex, 2)} EUR/kW")
    st.write(f"Allowable investment costs (with flexibility): {round(m.var_allowable_cost.value, 2)} EUR/kW")
    st.write(f"Benefit of flexibility: {round(m.var_allowable_cost.value - allowable_cost_noflex, 2)} EUR/kW")
